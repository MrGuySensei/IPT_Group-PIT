  ...
  return (
    <div style={{ minHeight: '100vh', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Lato', sans-serif" }}>
      ...
      <style>{`
        ...
        .signup-input:focus { border-color: #7c3aed; }
        ...
        .signup-btn:hover { background: #6d28d9; }
      ...
      {books.map((b, i) => (
        <div key={i} className="fbook" style={{ left: b.left, fontSize: b.size, animationDuration: b.dur, animationDelay: b.delay }}>{b.icon}</div>
      ))}
      ...
      <div style={{ position: 'relative', zIndex: 5, display: 'flex', width: '100%', maxWidth: '900px', alignItems: 'center', justifyContent: 'space-between', padding: '0 40px', boxSizing: 'border-box' }}>
        ...
        <div style={{ maxWidth: '320px' }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: '38px', fontWeight: '700', color: 'white', lineHeight: '1.2', margin: '0 0 14px 0' }}>
            Join our<br /><em style={{ color: '#a78bfa', fontStyle: 'italic' }}>community.</em>
          </h1>
        ...
        {/* Signup card */}
        <div style={{ width: '320px', background: 'rgba(6,10,22,0.9)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: '18px', padding: '32px 28px' }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: '20px', color: 'white', margin: '0 0 4px 0', fontWeight: '700' }}>Create Account</h2>
          <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', margin: '0 0 18px 0', fontWeight: '300' }}>Join the Library Borrowing System</p>

          {error && <div style={{ background: 'rgba(180,40,40,0.2)', border: '1px solid rgba(180,40,40,0.4)', color: '#f87171', borderRadius: '9px', padding: '10px 14px', marginBottom: '14px', fontSize: '12px' }}>❌ {error}</div>}
          {success && <div style={{ background: 'rgba(40,180,80,0.15)', border: '1px solid rgba(40,180,80,0.3)', color: '#86efac', borderRadius: '9px', padding: '10px 14px', marginBottom: '14px', fontSize: '12px' }}>✅ {success}</div>}

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '10px', color: '#a78bfa', fontWeight: '700', letterSpacing: '1.5px', marginBottom: '6px' }}>USERNAME</label>
              <input name="username" type="text" required placeholder="Username" value={form.username} onChange={handleChange} className="signup-input" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '10px', color: '#a78bfa', fontWeight: '700', letterSpacing: '1.5px', marginBottom: '6px' }}>EMAIL</label>
              <input name="email" type="email" placeholder="Optional" value={form.email} onChange={handleChange} className="signup-input" />
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '10px', color: '#a78bfa', fontWeight: '700', letterSpacing: '1.5px', marginBottom: '6px' }}>PASSWORD</label>
              <input name="password" type="password" required placeholder="Min. 6" value={form.password} onChange={handleChange} className="signup-input" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '10px', color: '#a78bfa', fontWeight: '700', letterSpacing: '1.5px', marginBottom: '6px' }}>CONFIRM</label>
              <input name="password2" type="password" required placeholder="Repeat" value={form.password2} onChange={handleChange} className="signup-input" />
            </div>
          </div>

          <button type="button" onClick={handleSubmit} disabled={loading} className="signup-btn">
            {loading ? 'CREATING...' : 'CREATE ACCOUNT'}
          </button>

          <p style={{ textAlign: 'center', fontSize: '12px', color: 'rgba(255,255,255,0.3)', fontWeight: '300', margin: 0 }}>
            Already have an account?{' '}
            <span onClick={() => navigate('/login')} style={{ color: '#a78bfa', fontWeight: '400', cursor: 'pointer' }}>Sign in here</span>
          </p>
        </div>
      </div>
    </div>
