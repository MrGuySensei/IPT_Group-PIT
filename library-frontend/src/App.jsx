...
import { dashboardApi, booksApi, borrowApi, profileApi, parseApiError } from './api'
...
const loginMember = async (username, password) => {
  ...
  const resendVerification = async (email) => {
    ...
    const data = await res.json()
    if (!res.ok) throw new Error(data.error || data.message || 'Could not resend email')
    return data
  }
...
function ResendVerificationPanel({ email, accentColor, onResendMessage }) {
  ...
  return (
    <div style={{
      ...
      background: 'var(--info-light)', border: '1px solid #b8d4eb',
    }}>
      <div style={{ fontSize: 13, color: 'var(--info)', marginBottom: 8 }}>
        Your account is not verified yet. Check your email inbox and spam folder, or resend the verification email below.
      </div>
      <button type="button" onClick={resend} disabled={busy || !email}
        style={{
          width: '100%', padding: '9px', borderRadius: 8, border: 'none', cursor: 'pointer',
          background: accentColor, color: 'white', fontWeight: 600, fontSize: 13,
          opacity: busy || !email ? 0.6 : 1, fontFamily: 'var(--font-body)',
        }}>
        {busy ? 'Sending…' : 'Resend verification email'}
      </button>
      {localMsg && (
        <div style={{ fontSize: 12, marginTop: 8, color: localMsg.includes('sent') ? 'var(--success)' : 'var(--danger)' }}>
          {localMsg}
        </div>
      )}
    </div>
...
const emptySignupForm = () => ({
  ...
  first_name: '', last_name: '', date_of_birth: '', profile_picture: null,
})
...
function StaffLoginPage({ onSwitchToMember }) {
  ...
  const set = (k) => (e) => {
    ...
    setError(''); setSuccess(''); setNeedsVerify(false)
  }
  ...
  const submit = async (e) => {
    e.preventDefault()
    setError(''); setSuccess(''); setNeedsVerify(false); setBusy(true)
    try {
      if (mode === 'login') {
        await loginStaff(form.username, form.password)
      } else {
        const data = await signupStaff(form)
      ...
      if (err.needsVerification) {
        setNeedsVerify(true)
        setVerifyEmail(err.email || (form.username.includes('@') ? form.username : form.email))
      }
    ...
    >
      ...
      <form onSubmit={submit}>
        <div className="form-grid">
          ...
          {mode === 'signup' && (
            <>
              ...
              <div>
                ...
                <input type="date" value={form.date_of_birth} onChange={set('date_of_birth')} required max={new Date().toISOString().split('T')[0]} />
              </div>
            </>
          )}
          <div>
          ...
          {success && (
            <div style={{ padding: '10px 14px', background: 'var(--success-light)', border: '1px solid #d4edda', borderRadius: 8, color: 'var(--success)', fontSize: 13 }}>
              ✅ {success}
            </div>
          )}
          {error && (
          ...
          {needsVerify && mode === 'login' && (
            <ResendVerificationPanel
              email={verifyEmail}
              accentColor="var(--accent)"
              onResendMessage={(msg) => setSuccess(msg)}
            />
          )}
...
function MemberBorrowModal({ book, user, onClose, onSuccess }) {
  ...
  const submit = async () => {
    if (!book.is_available) {
      setFormError('This book is currently unavailable.')
      return
    }
    if (!form.borrower_name?.trim() || !form.borrower_email?.trim() || !form.due_date) return
    setLoading(true)
    setFormError('')
    try {
      ...
      onSuccess(`"${book.title}" borrowed successfully!`)
      onClose()
    } catch (e) {
      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="form-grid">
          ...
          <div>
            ...
            <input type="email" value={form.borrower_email} readOnly style={{ background: 'var(--paper-warm)', cursor: 'not-allowed' }} />
          </div>
          <div>
        ...
        {formError && (
          <div style={{ padding: '10px 12px', background: 'var(--danger-light)', borderRadius: 8, color: 'var(--danger)', fontSize: 13 }}>
            {formError}
          </div>
        )}
        <div className="form-actions">
          <button className="btn btn-ghost" onClick={onClose}>Cancel</button>
          <button type="button" disabled={loading || !book.is_available || !form.borrower_name || !form.borrower_email || !form.due_date}
            onClick={submit}
...
function MemberDashboard() {
  ...
  const [availStats, setAvailStats] = useState({ total: 0, available: 0, unavailable: 0 })
  const [borrowBook, setBorrowBook] = useState(null)

  const loadAvailStats = useCallback(async () => {
    try {
      const [allRes, availRes, unavailRes] = await Promise.all([
        booksApi.list(),
        booksApi.list({ available: 'true' }),
        booksApi.list({ available: 'false' }),
      ])
      setAvailStats({
        total: allRes.data.length,
        available: availRes.data.length,
        unavailable: unavailRes.data.length,
      })
  ...
  }, [search, availFilter, loadAvailStats])
        ...
        {tab === 'browse' && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
              {[
                { label: 'Total in library', value: availStats.total, color: '' },
                { label: 'Available now', value: availStats.available, color: 'var(--success)' },
                { label: 'Unavailable', value: availStats.unavailable, color: availStats.unavailable > 0 ? 'var(--danger)' : '' },
              ].map(s => (
                <div key={s.label} className="stat-card">
                  <div className="stat-label">{s.label}</div>
                  <div className="stat-value" style={{ fontSize: 26, ...(s.color ? { color: s.color } : {}) }}>{s.value}</div>
                </div>
              ))}
            </div>

            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 16, alignItems: 'center' }}>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search title, author, ISBN…" style={{ maxWidth: 280, flex: '1 1 200px' }} />
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {[
                  ['all', `All (${availStats.total})`],
                  ['available', `Available (${availStats.available})`],
                  ['unavailable', `Unavailable (${availStats.unavailable})`],
                ].map(([id, label]) => (
                  <button key={id} type="button" onClick={() => setAvailFilter(id)} className="btn btn-sm"
                    style={{
                      background: availFilter === id ? (id === 'available' ? 'var(--success)' : id === 'unavailable' ? 'var(--danger)' : 'var(--ink)') : 'transparent',
                      color: availFilter === id ? 'white' : 'var(--ink-muted)',
                      border: '1.5px solid',
                      borderColor: availFilter === id ? 'transparent' : 'var(--border)',
                    }}>
                    {label}
                  </button>
                ))}
            ...
            {booksLoading ? <div className="spinner" /> : books.length === 0 ? (
              <div className="empty-state">
                ...
                <p>
                  {availFilter === 'available' ? 'No available books right now' :
                    availFilter === 'unavailable' ? 'No unavailable books' : 'No books found'}
                </p>
              </div>
            ) : (
              <div style={{ display: 'grid', gap: 8 }}>
                {books.map(b => (
                  <div key={b.id} className="card" style={{
                    ...
                    opacity: b.is_available ? 1 : 0.85,
                    borderLeft: b.is_available ? '3px solid var(--success)' : '3px solid var(--border)',
                  }}>
                    <div style={{
                      width: 44, height: 56, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 22,
                      background: b.is_available ? 'var(--success-light)' : 'var(--paper-warm)',
                    }}>📖</div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      ...
                      {b.is_available ? (
                        ...
                        <span style={{ fontSize: 11, color: 'var(--ink-muted)', maxWidth: 90, textAlign: 'right' }}>All copies borrowed</span>
                      )}
                    </div>
                  </div>
                ))}
...
function ProfileModal({ onClose }) {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        if (mounted) setError(err.response?.data?.detail || err.message || 'Unable to load profile')
      } finally {
  ...
  const age = profile?.age ?? getAge(profile?.profile?.date_of_birth)
  const borrowedBooks = profile?.borrowed_books || []
  const activeBooks = borrowedBooks.filter(b => b.status !== 'returned')
  const returnedBooks = borrowedBooks.filter(b => b.status === 'returned')

  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 620, width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
          <div>
            <div style={{ fontSize: 22, fontWeight: 700 }}>My Profile</div>
            <div style={{ color: 'var(--ink-muted)', fontSize: 13 }}>Your account details and borrowed books</div>
          </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gap: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--ink-faint)', marginTop: 4 }}>
                  {profile.profile?.total_books_borrowed ?? borrowedBooks.length} book{(profile.profile?.total_books_borrowed ?? borrowedBooks.length) !== 1 ? 's' : ''} borrowed total
                  {' · '}{profile.profile?.currently_borrowed ?? activeBooks.length} currently active
                </div>
              </div>
            </div>
            ...
            <div className="card" style={{ padding: '18px 20px' }}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 16, marginBottom: 14 }}>Borrowed Books</div>
              {borrowedBooks.length === 0 ? (
                <div style={{ fontSize: 13, color: 'var(--ink-muted)' }}>You have not borrowed any books yet.</div>
              ) : (
                <div style={{ display: 'grid', gap: 10 }}>
                  {activeBooks.length > 0 && (
                    <>
                      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--ink-muted)' }}>
                        Currently borrowed ({activeBooks.length})
                      </div>
                      {activeBooks.map(r => (
                        <div key={r.id} style={{ padding: '10px 12px', borderRadius: 8, background: 'var(--info-light)', borderLeft: r.status === 'overdue' ? '3px solid var(--danger)' : '3px solid var(--info)' }}>
                          <div style={{ fontWeight: 600, fontSize: 14 }}>{r.book_title}</div>
                          <div style={{ fontSize: 12, color: 'var(--ink-muted)' }}>{r.book_author}</div>
                          <div style={{ display: 'flex', gap: 12, marginTop: 6, fontSize: 12, color: 'var(--ink-muted)' }}>
                            <span className={`badge badge-${r.status}`}>{r.status}</span>
                            <span>Due: {fmt(r.due_date)}</span>
                          </div>
                        </div>
                      ))}
                    </>
                  ...
                  {returnedBooks.length > 0 && (
                    <>
                      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--ink-muted)', marginTop: activeBooks.length ? 8 : 0 }}>
                        Returned ({returnedBooks.length})
                      </div>
                      {returnedBooks.map(r => (
                        <div key={r.id} style={{ padding: '10px 12px', borderRadius: 8, background: 'var(--paper-warm)' }}>
                          <div style={{ fontWeight: 600, fontSize: 14 }}>{r.book_title}</div>
                          <div style={{ fontSize: 12, color: 'var(--ink-muted)' }}>{r.book_author}</div>
                          <div style={{ fontSize: 12, color: 'var(--ink-faint)', marginTop: 4 }}>
                            Borrowed {fmt(r.borrow_date)} · Returned {fmt(r.return_date)}
                          </div>
                        </div>
                      ))}
                    </>
